...
function renderNameLine(structure) {
  ...
  if(type === 'function' || (type === 'classMethod' && !isStatic)) {
    nameLine = ` * @function ${name}`;
  } else if (type === 'classMethod' && isStatic) {
    nameLine = ` * @static ${name}`;
  }
...
/**
 * renderDescriptionLine - renders a description line
 *
 * @param  {Object} structure Object describing the JS Doc
 * @return {String}           Description line for the JSDoc
 */
function renderDescriptionLine(structure) {
  const {
 ...
 * @description Renders an access line
 *
 * @return {String} accessLine Description
 */
function renderAccessLine() {
  return ' * @access private | public';
}
...
function jsdocifyParams(params) {
  return params.map(({
    type = 'type', name, defaultValue, parent,
  }) => {
    let tidiedName = name;
    if (parent) {
...
function renderFuncContent({ params = [], returns = {} }) {
  ...
  for (let i = 0; i < renderedParams.length; i += 1) {
...
export default function render(structure) {
  ...
  const descriptionLine = renderDescriptionLine(structure);
  const accessLine = renderAccessLine();
  const header = [OPEN];
  const footer = [CLOSE];
  ...
  const headerLines = [nameLine, descriptionLine];

  if(type === 'function' || type === 'classMethod') {
    headerLines.push(accessLine);
  }

  const content = headerLines.concat(contentBuilder(structure));
